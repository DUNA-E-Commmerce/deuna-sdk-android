...
fun PaymentWidgetBridge.saveBase64ImageToDevice(base64Image: String) {
    ...
    val tempFile = File(deunaWidget.context.cacheDir, filename)
    try {
        ...
        e.printStackTrace()
        Toast.makeText(deunaWidget.context, "Error al guardar la imagen", Toast.LENGTH_LONG).show()
        return
    }
    ...
    try {
        ...
        Toast.makeText(
            deunaWidget.context, "Imagen descargada: ${downloadFile.absolutePath}", Toast.LENGTH_LONG
        ).show()
